...
public class PermissionInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
       ...
       // List<Resource> userAllResourceList = (List<Resource>) request.getSession().getAttribute(SystemConst.USER_ALL_RESOURCE_LIST);
        ...
        String userAllResourceListJson = RedisUtil.get(KeyUtil.buildUserResourceListKey(sessionId));
        List<Resource> userAllResourceList = JSONObject.parseArray(userAllResourceListJson, Resource.class);
        // 获取用户访问的url
...
//        List<Resource> allResourceList = (List<Resource>) request.getSession().getAttribute(SystemConst.ALL_RESOURCE_LIST);
        String allResourceListJson = RedisUtil.get(KeyUtil.buildAllResourceListKey(sessionId));
        List<Resource> allResourceList = JSONObject.parseArray(allResourceListJson, Resource.class);
