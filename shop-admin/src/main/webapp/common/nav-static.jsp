    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="nav_menu">
            ...
            <ul class="nav navbar-nav navbar-right" id="userInfoDiv">
                ...
                <li style="display: none" id="loginTimeDiv"><a href="#">上次登录的时间##loginTime##</a></li>
...
<script>
    ...
    function initLoginInfo() {
        $.ajax({
            type:"post",
            url:"/user/findLoginInfo.jhtml",
            success:function (result) {
                if (result.code == 200) {
                    var v_data = result.data;
                    console.log(v_data.loginTime);
                    if (v_data.loginTime) {
                        var v_loginTimeHtml = $("#loginTimeDiv").html().replace(/##loginTime##/g, v_data.loginTimeStr);
                        $("#loginTimeDiv").html(v_loginTimeHtml);
                        $("#loginTimeDiv").show();
                    }
                    var v_userInfoMenu = $("#userInfoDiv").html().replace(/##userName##/g, v_data.realName)
                                                                 .replace(/##loginCount##/g, v_data.loginCount);
                    $("#userInfoDiv").html(v_userInfoMenu);
                }
            }
        })
    }
